...
def load_data(messages_filepath, categories_filepath):
    ...
    inputs:
    messages_filepath: string. "Messages.csv" dataset filepath.
    categories_filepath: string. "Categories.csv" dataset filepath.
       
    outputs:
    df: Dataframe that is a merged dataset of the "Messages" and "Categories" dataset.
    """
...
def clean_data(df):
    ...
    """Converts the df dataframe into a dataframe that be analyzed by a pipeline model.
    ...
    #create a column for each emergency message category
    categories = df['categories'].str.split(';', expand = True)
    
    #select first row
    row = categories.iloc[0]
    
    #get list of categories from that row by extracting the string untill second to last character
    category_colnames = row.transform(lambda x: x[:-2]).tolist()
    
    #rename cols to categories
    categories.columns = category_colnames
    
    # Convert  category strings to numbers by extracting the last value in the string (0 or 1)
    ...
    # Drop the  categories columns  from the df dataframe
...
def save_data(df, database_filename):
    ...
    None
    """
    create_engine('sqlite:///{}'.format(database_filename))
